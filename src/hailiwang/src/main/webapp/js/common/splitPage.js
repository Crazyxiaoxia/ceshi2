...
var common_splitPage = function() {
	...
	var splitPageHtml = function(divId, formId, totalRow, pageSize, pageNumber, totalPages, isSelectPage, isSelectSize, orderColunm, orderMode, currentPageCount){
		...
		splitStr += '<div class="dataTables_paginate paging_bootstrap" style="margin-right:45px;">';
		splitStr += '<ul class="pagination pagination-blue">';
		
		if (pageNumber == 1 || totalPages == 0) {
			splitStr += '<li class="prev disabled"><a href="javascript:void(0)" style="margin-top:5px;">' + i18n_common_splitPage_previous + '</a></li>';
		} else {
			splitStr += '<li class="prev"><a style="margin-top:5px;" href="javascript:common_splitPage.splitPageLink(\''+divId+'\', \''+formId+'\', ' + (pageNumber - 1) + ');">' + i18n_common_splitPage_previous + '</a></li>';
		}
		
		for (var i = 1; i <= totalPages; i++) {
	        if (i == 2 && pageNumber - 4 > 1) {
	        	splitStr += '<li><a href="javascript:void(0)" style="margin-top:5px;">...</a></li>';
	            i = pageNumber - 4;
	            ...
	            if (pageNumber == i) {
	            	splitStr += '<li class="active"><a  style="margin-top:5px;" href="javascript:void(0)" style="color: #272727; font-size: 14px; text-decoration: none;">' + pageNumber + '</a></li>';
	            } else {
	            	splitStr += '<li><a style="margin-top:5px;" href="javascript:common_splitPage.splitPageLink(\''+divId+'\', \''+formId+'\', ' + i + ');" style="color: #898989; font-size: 14px;">';
	            	splitStr += i;
	            	splitStr += '</a></li>';
	            }
	        }
		...
		if (pageNumber == totalPages || totalPages == 0) {
			splitStr += '<li class="next disabled"><a href="javascript:void(0)" style="margin-top:5px;" >' + i18n_common_splitPage_next + '</a></li>';
		} else {
			splitStr += '<li class="next"><a style="margin-top:5px;" href="javascript:common_splitPage.splitPageLink(\''+divId+'\', \''+formId+'\', ' + (pageNumber + 1) + ');">' + i18n_common_splitPage_next + '</a></li>';
		}
		
		if(isSelectPage == true){
			splitStr += '&nbsp;&nbsp;<li><select name="pageNumber" onChange="common_splitPage.splitPageLink(\''+divId+'\', \''+formId+'\', this.value);" style="width: 120px; height:35px;">';
			for (var i = 1; i <= totalPages; i++) {
				if (i == pageNumber) {
		...
		if(isSelectSize == true){
			splitStr += '<li><select name="pageSize" onChange="common_splitPage.splitPageLink(\''+divId+'\', \''+formId+'\', 1);" style="width: 100px; height:35px;">';
			
			var optionStr = '<option value="10" selected="selected">' + i18n_common_splitPage_perPage + '10' + i18n_common_splitPage_strip + '</option>';
